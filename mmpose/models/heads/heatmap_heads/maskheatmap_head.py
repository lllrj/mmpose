...
class MaskHeatmapHead(BaseHead):
    ...
    def add_prefix(self,inputs, prefix):
        """Add prefix for dict.
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tensor:
        ...
        x = feats[-1]
        if self.training:
            mask = torch.where(torch.rand_like(x) > 0.5, torch.tensor(1.).cuda(), torch.tensor(0.).cuda())
            mask_x = torch.mul(x, mask).cuda()
            mask_x = self.deconv_layers(mask_x)
            mask_x = self.conv_layers(mask_x)
            mask_x = self.final_layer(mask_x)
            
        x = self.deconv_layers(x)
        x = self.conv_layers(x)
        x = self.final_layer(x)
        # print('training ?', self.training) flase
        return (x, mask_x) if self.training else x
    ...
    def loss(self,
        ...
        """
        pred_fields, mask_pred_fields = self.forward(feats)
        ...
        maskloss = self.loss_module(mask_pred_fields, gt_heatmaps, keypoint_weights)

        losses.update(loss_kpt=loss)
        losses.update(maskloss_kpt=maskloss)
