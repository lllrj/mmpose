...
class DS(nn.Module):
    ...
    def __init__(self, sample_ratio, embed_dim, dim_out, stride = 2):
        super().__init__()
    ...
    def forward(self, token_dict):
        ...
        x = self.conv(token_dict)# C翻倍
        x = self.norm(x)
        token_dict['x'] = x
        B, N, C = x.shape
        x = x.permute(0, 2, 1).reshape(B, C, H, W)
        x = self.dowmsample(x)# H W缩减
        # print('-------------------------------------------')
        ...
        down_dict = {}
        down_dict['x'] = x
        down_dict['token_num'] = x.shape[1]
        # H, W = token_dict['map_size']
        H = math.floor((H - 1) / 2 + 1)
        W = math.floor((W - 1) / 2 + 1)
        down_dict['map_size'] = [H, W]
        down_dict['init_grid_size'] = [H, W]
        down_dict['idx_token'] = 0# 无意义【
        down_dict['agg_weight']  = x.new_ones(B, x.shape[1], 1)

        return down_dict, token_dict

@BACKBONES.register_module()
class FdsFormer(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(1, num_stages):
            ...
            ds = DS(sample_ratios[i - 1], embed_dims[i - 1], embed_dims[i])
            patch_embed = PatchEmbed(
                in_channels=in_channels[i],
                embed_dims=embed_dims[i],
                kernel_size=1,
    ...
    def forward(self, x):
        ...
        for j in range(4):
            ...
            tem, (H, W) = patch_embed(x[j])# embed has norm last
            temp.append(tem)
            shp.append((H, W))
