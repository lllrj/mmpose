...
def index_points(points, idx):
    ...
    batch_indices = torch.arange(
        B, dtype=torch.long).to(device).view(view_shape).repeat(repeat_shape)# (B, S) = [[0, ..., 0], [1, ..., 1], ...,[B-1, ..., B-1]]
...
def token_interp(target_dict, source_dict):
    ...
    weight = weight.reshape(-1) # agg_weight.shape = (B,N,1) 平铺至一维

    # choose the way with fewer flops. 
    if N_init < T * S: # t最初的N < num_t * source当前的N
        # use sparse matrix multiplication
        # Flops: B * N_init * (C+2)
...
def merge_tokens(token_dict, idx_cluster, cluster_num, token_weight=None):
    ...
    Return:
        out_dict (dict): dict for output token information out_dict['x'] (tensor[B, N/4, C])
    """

    x = token_dict['x']
    idx_token = token_dict['idx_token'] # 此时还是上个stage的idx_token(B, N)
    ...
    # idx_cluster->(B, N) idx_cluster[..., None]->(B, N, 1); 
